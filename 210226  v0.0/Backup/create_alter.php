<?php
	...
	// ver 2: Return all alters and sampling in js
	function GetAltersToCheck(){
		// Function外定義的變數需經過global定義才能直接在function內使用
		global $db;
		global $ego_id; 
		$ego_id = $_SESSION['acc_info']['id'];
		//query other alters to show in table
		$sql  = " SELECT alter_id, alter_name FROM alter_list where ego_id = :v1 ";
		$stmt = $db->prepare($sql);
		...
		$rs_other_alters = json_encode($json,JSON_UNESCAPED_UNICODE);
		echo $rs_other_alters;		
	}
	...
	if(isset($_POST['FormSubmit'])) {
		...
		// $alter_tbl 		= empty($_POST['alter_tbl'])  ? null :json_encode(array_filter($_POST['alter_tbl']), true);
		$alter_tbl 		= empty($_POST['alter_tbl'])  ? null :$_POST['alter_tbl'];
		$index_duplicate = $_SESSION['acc_info']['id'] ."_". $alter_name;
		...
		$stmt = $db->prepare($sql);
		$stmt->bindParam(':v1', $ego_id);
		$stmt->bindParam(':v2', $alter_name);
		$stmt->bindParam(':v3', $alter_nickname);
		$stmt->bindParam(':v4', $group_info);
		$stmt->bindParam(':v5', $relationship);
		$stmt->bindParam(':v6', $relationship_other);
		$stmt->bindParam(':v7', $seniority);
		$stmt->bindParam(':v8', $gender);
		$stmt->bindParam(':v9', $age);
		$stmt->bindParam(':v10', $job);
		$stmt->bindParam(':v11', $edu);
		$stmt->bindParam(':v12', $marriage);
		$stmt->bindParam(':v13', $city);
		$stmt->bindParam(':v14', $town);
		$stmt->bindParam(':v15', $res_other);
		$stmt->bindParam(':v16', $know_howlong);
		$stmt->bindParam(':v17', $meet_freq);
        $stmt->bindParam(':v21', $contact);
		$stmt->bindParam(':v18', $is_rel_group);
		$stmt->bindParam(':v19', $index_duplicate);
		$stmt->bindParam(':v20', $now);
	...
	<script type="text/javascript">
		$(document).ready(function() {
	    		...
	    		$.ajax({ 
					...
					data: {
						...
						if (data[0] == "SubmitSuccess") {
							$.confirm({
								title: '',
							    content: '接觸對象資料填寫完成！<br>接著填寫接觸日記嗎？',
							    buttons: {
							    	'稍後再填': function() {
			...
			$("#back").on('click', function(evt) {
				window.location.href = 'http://cdiary3.tw/main_tdiary.php';
			})
		})
...
<body>
	...
	<form id="form" class="form-horizontal">
		<div class="card card-primary">
			<div class="card-header">新增接觸對象</div>
			<div class="card-body">
				<div class="input-group">
					<div class="input-group-prepend col-sm-4 pl-0 pr-0">
				  	...
				  	<div class="col-sm-8 pl-0 pr-0">
						<input type="text" class="form-control" name="alter_name" placeholder="建議填寫對方在LINE中的名稱或您能辨識記憶的名字" required>
					</div>
				...
				<div class="input-group">
					<div class="input-group-prepend col-sm-4 pl-0 pr-0">
				    	<span class="input-group-text"><span class="alter_name_fill"></span>是否也在我參與的宗教群組中(可複選)</span>
				  	</div>
				  	<div class="col-sm-8 pl-0 pr-0">
                ...
                <div class="input-group">
					<div class="input-group-prepend col-sm-4 pl-0 pr-0">
				    	<span class="input-group-text">整體而言，喜不喜歡和<span class="alter_name_fill"></span>互動</span>
				  	</div>
					<div class="btn-group btn-group-toggle col-sm-8 pl-0 pr-0" data-toggle="buttons">
