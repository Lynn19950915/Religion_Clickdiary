<?php
	...
	// check if exist, direction to main.php
	// $sql_query = "SELECT * FROM profile_v2 WHERE id=:v1";
	// $stmt=$db->prepare($sql_query);
	// $stmt->bindParam(':v1', $_SESSION['id']);
	// $stmt->execute();
	// $n = $stmt->rowCount();
	
	// 已填過profile導向main.php,欲修改請使用edit_profile
	// if($n > 0){ 
	...
	if(isset($_POST['FormSubmit'])){
		...
		try {
			...
			$sql  = "UPDATE `account` SET `reg_status` = 2 WHERE id = :v1";
			$stmt = $db->prepare($sql);
			$stmt->bindParam(':v1', $_SESSION['acc_info']['id']);
			$stmt->execute();
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("#form").on("submit", function (event) {
				...
				var rel_donate_buddhism 	= $("input[name='rel_donate_buddhism']").attr('checked', "true")?1 :0;
				var rel_donate_christianity = $("input[name='rel_donate_christianity']").attr('checked', "true")?1 :0;
				var rel_donate_folkbelief 	= $("input[name='rel_donate_folkbelief']").attr('checked', "true")?1 :0;
				var rel_donate_taoism 		= $("input[name='rel_donate_taoism']").attr('checked', "true")?1 :0;
				var rel_donate_other_chk 	= $("input[name='rel_donate_other_chk']").attr('checked', "true")?1 :0;
				var rel_donate_other 		= $("input[name='rel_donate_other']").val()
				var rel_vol_buddhism 		= $("input[name='rel_vol_buddhism']").attr('checked', "true")?1 :0;
				var rel_vol_christianity 	= $("input[name='rel_vol_christianity']").attr('checked', "true")?1 :0;
				var rel_vol_folkbelief 		= $("input[name='rel_vol_folkbelief']").attr('checked', "true")?1 :0;
				var rel_vol_taoism 			= $("input[name='rel_vol_taoism']").attr('checked', "true")?1 :0;
				var rel_vol_other_chk 		= $("input[name='rel_vol_other_chk']").attr('checked', "true")?1 :0;
				var rel_vol_other 			= $("input[name='rel_vol_other']").val()
				var rel_pb_biblestudy 		= $("input[name='rel_pb_biblestudy']").attr('checked', "true")?1 :0;
				var rel_pb_meditate 		= $("input[name='rel_pb_meditate']").attr('checked', "true")?1 :0;
				var rel_pb_worship 			= $("input[name='rel_pb_worship']").attr('checked', "true")?1 :0;
				var rel_pb_pray 			= $("input[name='rel_pb_pray']").attr('checked', "true")?1 :0;
				var rel_pb_other_chk 		= $("input[name='rel_pb_other_chk']").attr('checked', "true")?1 :0;
				...
				$("input[name='group_name']").each(function(){
					...
					group_list.group_is_religious.push(999)
				})
		...
		<form id="form" >
			<fieldset >
				...
				<div id="page_2" class="hide_atfirst">
					...
					<div class="input-group mb-2">
						<div class="input-group-prepend col-sm-12">
					    	<div class="input-group-text col-sm-12 text-left">
					    		<span>
					    			我目前固定查閱、瀏覽或發文的<br class="d-sm-none">宗教網路社群有哪些(例如：Line、Facebook社團)
					    		</span>
					    	</div>
						</div>
					</div>
					<div class="input-group group-info mt-2 mb-2">
						<div class="group-info-wrapper col-sm-12 pl-0 pr-0 mt-2 mb-2 border border-warning">
							<div class="input-group-prepend col-sm-12">
						    	<span class="input-group-text">群組名稱</span>
						    	<input type="text" class="form-control" name="group_name" required>
							</div>
							<div class="input-group-prepend col-sm-12">
						    	<span class="input-group-text">我在群組中使用的名字</span>
						    	<input type="text" class="form-control" name="group_myname" required>
							</div>
							<div class="input-group-prepend col-sm-12">
						    	<span class="input-group-text">查閱頻率</span>
						    	<div class="btn-group btn-group-toggle btn-group-custom-flex pr-0" data-toggle="buttons">
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_check_freq_0" class="group_check_freq" value="0"> 每天多次
								  </label>
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_check_freq_0" class="group_check_freq" value="1"> 每天一次
								  </label>
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_check_freq_0" class="group_check_freq" value="2"> 兩三天一次
								  </label>
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_check_freq_0" class="group_check_freq" value="3"> 偶爾
								  </label>
								</div>
							</div>
							<!-- <div class="input-group-prepend col-sm-12">
						    	<span class="input-group-text">是否為宗教群組</span>
						    	<div class="btn-group btn-group-toggle btn-group-custom-flex" data-toggle="buttons">
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_is_religious_0" class="group_is_religious" value="1"> 是
								  </label>
								  <label class="btn btn-outline-primary">
								    <input type="radio" name="group_is_religious_0" class="group_is_religious" value="0"> 否
								  </label>
								</div>
							</div> -->
							<div class="input-group-prepend col-sm-12">
						    	<span class="input-group-text text-left">我曾邀請多少人<br class="d-sm-none">加入此群組</span>
						    	<div class="btn-group btn-group-toggle btn-group-custom-flex pr-0" data-toggle="buttons">
								  <label class="btn btn-outline-primary pt-onss">
								    <input type="radio" name="group_n_invitees_0" class="group_n_invitees" value="0"> 0人
								  </label>
								  <label class="btn btn-outline-primary pt-onss">
								    <input type="radio" name="group_n_invitees_0" class="group_n_invitees" value="1"> 1 - 4人
								  </label>
								  <label class="btn btn-outline-primary pt-onss">
								    <input type="radio" name="group_n_invitees_0" class="group_n_invitees" value="2"> 5人以上
								  </label>
								</div>
							</div>
						</div>
					...
					<div class="input-group-prepend col-sm-12 pl-0 pr-0">
						<div class="col-sm-4 pl-0 pr-0" style="align-self: center;">
							<label class="control-label text-success">請透過右方按鈕<br class="d-sm-none">增減群組</label>
						</div>
						<div class="col-sm-6 pl-0 pr-0">
							<button id="group-add"   class="btn" style="font-size: 2em"><i class="fas fa-plus-circle"></i></button>
							<button id="group-minus" class="btn" style="font-size: 2em"><i class="fas fa-minus-circle"></i></button>	
						</div>	
					</div>
					<hr>
					<div align="center">
